...
import { index as perfisIndex, create as perfisCreate } from '@/routes/perfis';
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-full max-w-[1920px] space-y-8 px-6 py-8 lg:px-8">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="space-y-1">
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-3xl font-bold tracking-tight text-foreground">Perfis</h1>
                    </div>
                    <p class="text-base text-muted-foreground">
                        Gerencie os perfis de acesso do sistema
                    </p>
                </div>
                <Link :href="perfisCreate().url">
            ...
            <Card class="border-border shadow-sm">
                ...
                <Table>
                    ...
                    <TableBody>
                        ...
                        <TableRow v-for="role in perfis.data" :key="role.id">
                            ...
                            <TableCell class="text-right">
                                <div class="flex justify-end gap-2">
                                    <Link :href="RoleController.edit(role).url">
            ...
            <!-- Pagination -->
            <div
                v-if="perfis.last_page > 1"
                class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"
            >
                <div class="text-sm text-muted-foreground">
                    Mostrando {{ perfis.data.length }} de {{ perfis.total }} perfis
                </div>
                <div class="flex gap-2">
                    <Link
                        v-for="link in perfis.links"
                        :key="link.label"
                        :href="link.url || '#'"
                        :class="[
        ...
        <Dialog v-model:open="showDeleteDialog">
            <DialogContent v-if="roleToDelete" class="sm:max-w-md">
                <Form
                    v-bind="RoleController.destroy.form(roleToDelete)"
                    :options="{
                        preserveScroll: true,
                    }"
                    @success="() => {
                    ...
                    @error="toast.error('Erro ao excluir perfil', 'Tente novamente mais tarde.')"
                    class="space-y-6"
                    v-slot="{ processing }"
                >
                    <DialogHeader class="space-y-2">
                        ...
                        <DialogDescription class="text-gray-600 pt-2">
                            ...
                            <strong class="font-medium text-gray-800">{{ roleToDelete.name }}</strong>.
                    ...
                    <div class="p-4 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
                        ...
                        <p>Confirme se este é o perfil correto antes de prosseguir.</p>
                    </div>
